...
const LANGKAH_STEPPER = [
  ...
  {
    label: "Estimasi",
    statuses: ["estimasi_pemakaian", "konfirmasi_pemakaian", "proses_potong"],
  },
  {
    label: "Buku Potong",
    statuses: ["input_buku_potong", "input_nota", "review_hpp"],
  },
        ...
        return (
          <div key={l.label} className="flex items-center shrink-0">
            ...
            {i < LANGKAH_STEPPER.length - 1 && (
              <div
                className={[
                  "h-0.5 w-6 mb-4 mx-0.5",
                  done ? "bg-gold-500" : "bg-champagne-200",
                ].join(" ")}
              />
            )}
          </div>
      ...
      <div className="bg-navy-900 px-4 pt-5 pb-0">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            <h1 className="font-heading text-heading text-champagne-100">
              {kode.kode_desain}
            </h1>
            <StatusBadge status={status} />
          </div>
          {kode.harga_jual_target && (
            <p className="font-sans text-label text-gold-500">
              {formatRp(kode.harga_jual_target)}
            </p>
          )}
        </div>
        ...
        {isDeera && (
          <StatusAksiPanel
            ...
            onLanjutDariBatalkan={() =>
              lanjutDariBatalkanMut.mutate(kode.status_sebelum_dibatalkan)
            }
            onMulaiInputBP={() =>
              mulaiInputBPMut.mutate(null, {
                onSuccess: () => navigate("/kode/" + kodeId + "/buku-potong"),
              })
            }
            onLanjutKeNota={() =>
              lanjutKeNotaMut.mutate(null, {
                onSuccess: () => navigate("/nota"),
              })
            }
            isPending={
              konfirmasiEstimasiMut.isPending ||
              lanjutProsesPotongMut.isPending ||
              batalkanMut.isPending ||
    ...
    <div className="space-y-4">
      {bisaApprove && (
        <div className="flex gap-3">
          <button
            onClick={() => onApprove(sampelAktif.id)}
            disabled={isPending}
          ...
          <button
            onClick={() => onTolakOpen(sampelAktif.id)}
    ...
    <div className="space-y-4">
      {nota?.status === "ditolak" && (
        <div className="rounded-xl bg-danger/10 border border-danger/20 px-4 py-3">
          <p className="font-sans text-label font-semibold text-danger">
            NOTA BIAYA DITOLAK
          </p>
          {nota.alasan_tolak && (
            <p className="mt-1 font-sans text-label text-danger">
              {nota.alasan_tolak}
            </p>
          )}
        </div>
...
function TabProduksi({ kode, isDeera, onUpdatePcs, onCatatReject }) {
  ...
  if (!["produksi", "siap_kirim", "selesai"].includes(kode.status)) {
    return (
      <p className="py-8 text-center font-sans text-body text-charcoal-300">
        Tracking produksi tersedia setelah HPP disetujui.
      </p>
    );
  }
  return (
  ...
  return (
    <div className="rounded-xl bg-surface border border-border overflow-hidden">
      <div className="bg-navy-900 px-4 py-3 flex justify-between items-center">
        <p className="font-sans text-label font-semibold text-champagne-100">
          {ukuran.ukuran}
        </p>
      ...
      <div className="divide-y divide-border">
        ...
        {warnaList.length === 0 && (
          <p className="px-4 py-3 font-sans text-label text-charcoal-300">
...
function WarnaTracking({ warna, isDeera, onUpdatePcs, onCatatReject }) {
  ...
  function submitReject() {
    ...
    onCatatReject({
      ...
      bahanTersedia:
        rejectForm.nasib === "produksi_ulang" ? rejectForm.bahanTersedia : null,
    ...
    <div className="px-4 py-3 space-y-2">
      ...
      <div className="space-y-1.5">
        {TAHAP_ORDER.map((tahap) => {
          ...
          const pct =
            totalPcs > 0
              ? Math.min(100, Math.round((pcsDone / totalPcs) * 100))
              : 0;
          const rejectCount = (t?.tracking_reject ?? []).reduce(
            (s, r) => s + r.pcs_reject,
            0,
          );
          return (
                  ...
                  {isDeera && trackingId && (
                    <div className="flex gap-1">
                      <button
                        ...
                        className="rounded px-1.5 py-0.5 font-sans text-xs bg-champagne-200 text-navy-900"
                      >
                        EDIT
                      </button>
                      <button
                        onClick={() => setRejectFor({ trackingId, tahap })}
                        className="rounded px-1.5 py-0.5 font-sans text-xs bg-danger/10 text-danger"
                      >
                        REJECT
                      </button>
                    </div>
              ...
              <div className="h-1.5 w-full rounded-full bg-champagne-200">
                <div
                  className={[
                    ...
                    pct === 100 ? "bg-gold-500" : "bg-navy-700",
                  ].join(" ")}
                  style={{ width: pct + "%" }}
                />
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-end bg-black/60">
          <div className="w-full rounded-t-2xl bg-surface px-4 pt-6 pb-8 space-y-4">
            <p className="font-heading text-heading text-navy-900">
              UPDATE {TAHAP_LABEL[editTahap.tahap].toUpperCase()}
            </p>
            <p className="font-sans text-label text-charcoal-600">
              Warna: {warna.nama_warna} \u2014 Total {totalPcs} pcs
            </p>
            <div className="space-y-1">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={submitPcs}
                className="flex-1 rounded-xl bg-gold-500 py-3.5 font-sans text-body font-semibold text-navy-900"
              >
                SIMPAN
              </button>
            </div>
      ...
      {rejectFor && (
        <div className="fixed inset-0 z-[60] flex items-end bg-black/60">
          <div className="w-full rounded-t-2xl bg-surface px-4 pt-6 pb-8 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <p className="font-sans text-label text-charcoal-600">
              Tahap: {TAHAP_LABEL[rejectFor.tahap]} \u2014 {warna.nama_warna}
            </p>
            <div className="space-y-3">
              <div className="space-y-1">
                <label className="font-sans text-xs font-semibold text-charcoal-600 uppercase">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRejectForm((p) => ({ ...p, pcs: e.target.value }))
                  }
                  className="w-full rounded-xl border border-border px-4 py-3 font-sans text-body text-navy-900 outline-none focus:border-gold-500"
                />
              </div>
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRejectForm((p) => ({
                      ...p,
                      alasan: e.target.value.toUpperCase(),
                    }))
                  }
                ...
                <div className="flex gap-2">
                  {NASIB_OPTIONS.map((o) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setRejectForm((p) => ({ ...p, nasib: o.value }))
                      }
                      className={[
                ...
                <div className="flex items-center justify-between rounded-xl border border-border px-4 py-3">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setRejectForm((p) => ({
                        ...p,
                        bahanTersedia: !p.bahanTersedia,
                      }))
                    }
                    className={[
                      "relative w-10 h-5 rounded-full transition-colors",
                      rejectForm.bahanTersedia
                        ? "bg-gold-500"
                        : "bg-charcoal-300",
                    ].join(" ")}
                  >
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRejectForm((p) => ({
                      ...p,
                      catatan: e.target.value.toUpperCase(),
                    }))
                  }
...
function ModalUploadSampel({ kode, onSubmit, onClose, isPending }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!fotoDepan && !fotoBelakang) {
      setErrMsg("Upload minimal satu foto (depan atau belakang).");
      return;
    }
    setErrMsg("");
    setUploading(true);
    try {
      ...
      onSubmit({
        foto_depan_url: depan?.url ?? null,
        foto_belakang_url: belakang?.url ?? null,
      });
    } catch (err) {
  ...
  const adaFoto = fotoDepan || fotoBelakang;
  return (
    <div className="fixed inset-0 z-[60] flex items-end bg-black/60">
      <div className="w-full rounded-t-2xl bg-surface px-4 pt-6 pb-8 space-y-4 max-h-[90vh] overflow-y-auto">
        <p className="font-heading text-heading text-navy-900">UPLOAD SAMPEL</p>
        <p className="font-sans text-label text-charcoal-600">
          Upload foto tampak depan & belakang (keduanya opsional \u2014 isi
          minimal satu).
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            {[
              ["Depan", fotoDepan, setFotoDepan, refDepan],
              ["Belakang", fotoBelakang, setFotoBelakang, refBelakang],
            ].map(([label, foto, setFoto, ref]) => (
              <div key={label}>
                <p className="font-sans text-xs font-semibold text-charcoal-600 uppercase mb-1.5">
                  {label}
                </p>
                <button
                  type="button"
                  ...
                  className={[
                    "aspect-square w-full rounded-xl border-2 border-dashed flex items-center justify-center overflow-hidden",
                    foto ? "border-gold-500" : "border-charcoal-300",
                  ].join(" ")}
                >
                  {foto ? (
                    <img
                    ...
                    <span className="font-sans text-2xl text-charcoal-300">
                      +
                    </span>
                  )}
                </button>
                <input
                  ...
                  accept="image/*"
                  className="hidden"
                  onChange={(e) => pickFile(e, setFoto)}
                />
              </div>
            ))}
          </div>
          {busy && (
            <p className="text-center font-sans text-label text-charcoal-600">
              Mengupload foto...
            </p>
          )}
          {errMsg && (
            <p className="font-sans text-label text-danger">{errMsg}</p>
          )}
          <div className="flex gap-3">
            <button
              type="button"
...
// ─── StatusAksiPanel ──────────────────────────────────────────────────────────
function StatusAksiPanel({
  status,
  statusSebelum,
  kodeId,
  navigate,
  onKonfirmasiEstimasi,
  onLanjutProsesPotong,
  onBatalkan,
  onLanjutDariBatalkan,
  onMulaiInputBP,
  onLanjutKeNota,
  ...
  const btn = (label, onClick, variant = "primary") => (
    <button
      ...
      disabled={isPending}
      className={[
        "flex-1 rounded-xl py-3 font-sans text-label font-semibold disabled:opacity-50 transition-colors",
        variant === "primary"
          ? "bg-gold-500 text-navy-900"
          : variant === "secondary"
            ? "border border-border text-charcoal-600 bg-surface"
            : variant === "danger"
  ...
  if (status === "estimasi_pemakaian") {
    rows.push(
      <div key="est" className="flex gap-2">
        {btn("KONFIRMASI ESTIMASI", onKonfirmasiEstimasi)}
      </div>,
    );
  }
  if (status === "konfirmasi_pemakaian") {
    rows.push(
      <div key="konfirm" className="flex gap-2">
  ...
  if (status === "proses_potong") {
    rows.push(
      <div key="potong" className="flex gap-2">
        {btn("INPUT BUKU POTONG", onMulaiInputBP)}
      </div>,
    );
  }
  if (status === "input_buku_potong") {
    rows.push(
      <div key="bp" className="flex gap-2">
        <button
          onClick={() => navigate("/kode/" + kodeId + "/buku-potong")}
  ...
  if (status === "input_nota") {
    rows.push(
      <div key="nota" className="flex gap-2">
  ...
  if (status === "dibatalkan" && statusSebelum) {
    rows.push(
      <div key="batal" className="flex gap-2">
        {btn("REVISI & LANJUTKAN", onLanjutDariBatalkan)}
      </div>,
    );
  }
  ...
  return (
    <div className="px-4 py-3 border-t border-white/10 space-y-2">{rows}</div>
  );
}
...
function ErrorScreen({ onBack }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-champagne-100 px-4">
      <p className="font-sans text-body text-charcoal-600 text-center">
        Kode tidak ditemukan atau gagal dimuat.
      </p>
      <button
        onClick={onBack}
        className="rounded-xl bg-navy-900 px-6 py-3 font-sans text-body font-semibold text-champagne-100"
      >
        KEMBALI
      </button>
    </div>
